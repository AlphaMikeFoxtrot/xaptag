...
// 		echo json_encode($scanned_directory);
	...
	$app->post("/user/visits", function(Request $request, Response $response) {
		$connection = mysqli_connect('mysql.hostinger.in', 'u347889504_xap2', 'xaptag', 'u347889504_xap2');
		$data = $request->getParsedBody();
		$id = $data['id'];
		$sql_get_visits = "SELECT * FROM `visit` WHERE `visit`.`user_id`='$id' order by `visit`.`timestamp_int` ASC;";
		...
		for($i = 0; $i < count($visits); $i++){
		    $docName = $visits[$i]['doctor_name'];
		    $time = $visits[$i]['timestamp'];
		    $visit = [$docName, $time];
		    $_visits[] = $visit;
		}
		echo json_encode($_visits);
	});
	...
	$app->post('/scan', function(Request $request, Response $response) {
		...
		if($u_id_check && $d_id_check) {
			...
			$sql = "INSERT INTO `visit` VALUES('$v_id', '$u_id', '$d_id', '$d_n', '$date', '$time');";
			...
			if($response_sql && $response_log) {
				...
				*/
				...
				// 4.
				$sql_get_visits = "SELECT * FROM `visit` WHERE `visit`.`user_id`='$u_id' order by `visit`.`timestamp_int` ASC;";
				$response_get_visits = mysqli_query($connection, $sql_get_visits);
				$visits = array();
				while($row = mysqli_fetch_assoc($response_get_visits)) {
					$visits[] = $row;
